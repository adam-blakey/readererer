...
- Validation errors are rendered beside their own field, so each form also carries `<x-forms.error-summary :fields="..." />` at the top: it lists the messages whose key has no field to sit next to (a field the controller removed from the generic form, a rule on something that is not a field, a `withErrors()` from outside the form request), which would otherwise be swallowed silently. `unhandled_error_messages()` in `helpers.php` does the matching; `fields` is the list of field names that form renders, and takes wildcards (`term_dates.*.start_datetime`). Add the component whenever you add a form.
