...
test('a setup group can be created with van drivers', function () {
    ...
    $response = $this->actingAs(make_user(UserRole::Admin))->post(route('setupgroups.store'), [
        'name' => 'Group A',
        'week' => 2,
        'color' => 'blue',
...
test('a setup group can be created without van drivers', function () {
    $this->actingAs(make_user(UserRole::Admin))->post(route('setupgroups.store'), [
        'name' => 'Group B',
        'color' => 'red',
    ])->assertRedirect();
...
test('updating a setup group replaces its attributes and van drivers', function () {
    ...
    $response = $this->actingAs(make_user(UserRole::Admin))->patch(route('setupgroups.update', $setupGroup), [
        'name' => 'Group Z',
        'week' => 3,
        'color' => 'green',
...
test('updating a setup group without van drivers removes them all', function () {
    ...
    $this->actingAs(make_user(UserRole::Admin))->patch(route('setupgroups.update', $setupGroup), [
        'name' => 'Group A',
        'week' => 1,
        'color' => 'blue',
    ]);
...
test('an invalid update leaves the setup group untouched', function () {
    ...
    $this->actingAs(make_user(UserRole::Admin))
        ->patch(route('setupgroups.update', $setupGroup), [
            'name' => 'Group Z',
            'week' => 99,
            'color' => 'green',
            'van_drivers' => [999999],
...
test('the setup group show and create pages render', function () {
    ...
    $this->actingAs($user)->get(route('setupgroups.create'))->assertOk();
    $this->actingAs($user)->get(route('setupgroups.edit', $setupGroup))->assertOk();
});
